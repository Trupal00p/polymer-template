...
class NDBEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if isinstance(obj, ndb.Model):
            ...
            # Each BlobKeyProperty is represented as a dict of
            ...
            for (name, prop) in obj._properties.iteritems():
                if isinstance(prop, ndb.BlobKeyProperty):
                    ...
                    # e.g. /api/my_model/<SOME_KEY>/blob_prop
                    blob_property_url = 'http://%s%s/%s/%s' % (
                        server_host, obj.RESTMeta.base_url, self._decode_key(obj.key), name)
                    obj_dict[name] = {
                        'upload_url': blob_property_url,
                        # Display as null if the blob property is not set
                        'download_url': blob_property_url if getattr(obj, name) else None
                    }

            # Filter the properties that will be returned to user
            included_properties = get_included_properties(obj, 'output')
            obj_dict = dict(
                (k, v) for k, v in obj_dict.iteritems() if k in included_properties)
...
def get_translation_table(model, input_type):
    ...
    translation_table.update(
        getattr(model.RESTMeta, 'translate_%s_property_names' % input_type, {}))
...
def translate_property_names(data, model, input_type):
    ...
    for old_name, new_name in translation_table.iteritems():
        if input_type == 'output' and old_name not in data:
            continue
        if input_type == 'input' and new_name not in data:
            continue
...
def get_included_properties(model, input_type):
    ...
    if meta_class:
        included_properties = set(
            getattr(meta_class, 'included_%s_properties' % input_type, []))
        included_properties.update(
            set(getattr(meta_class, 'included_properties', [])))

    if not included_properties:
        # No Meta class (or no included properties defined), assume all
        # properties are included
        included_properties = set(model._properties.keys())

    if meta_class:
        excluded_properties = set(
            getattr(meta_class, 'excluded_%s_properties' % input_type, []))
        excluded_properties.update(
            set(getattr(meta_class, 'excluded_properties', [])))
    else:
    ...
    if input_type == 'input':
        excluded_properties.update(
            set(BaseRESTHandler.DEFAULT_EXCLUDED_INPUT_PROPERTIES))
        if meta_class and getattr(meta_class, 'use_input_id', False):
            included_properties.update(['id'])
    if input_type == 'output':
        excluded_properties.update(
            set(BaseRESTHandler.DEFAULT_EXCLUDED_OUTPUT_PROPERTIES))
...
class BaseRESTHandler(webapp2.RequestHandler):
    ...
    # The default number of results to return for a query in case `limit`
    # parameter wasn't provided by the user
    ...
    DEFAULT_EXCLUDED_INPUT_PROPERTIES = ['class_']
    ...
    def dispatch(self):
        ...
        try:
            if getattr(self, 'allow_http_method_override', False) and ('X-HTTP-Method-Override' in self.request.headers):
                ...
                overridden_method_name = self.request.headers[
                    'X-HTTP-Method-Override'].upper().strip()
    ...
    def get_response(self, status, content):
        ...
        response.headers[
            'Access-Control-Allow-Methods'] = ', '.join(self.permissions.keys())
    ...
    def _filter_query(self):
        ...
        try:
            ...
            for original_name, new_name in translation_table.iteritems():
                # Replace any references to the new property name with the old
                # (original) one
                query = re.sub(r'\b%s\s*(<=|>=|=|<|>|!=|(\s+IN\s+))' %
                               new_name, r'%s \1' % original_name, query, flags=re.IGNORECASE)

            return self.model.gql('WHERE ' + query)
        except Exception, exc:
            # Invalid query
            raise RESTException(
                'Invalid query param - "%s"' % self.request.GET.get('q'))
    ...
    def _fetch_query(self, query):
        ...
        if not self.request.GET.get('limit'):
            ...
            try:
                ...
                # Invalid limit value
                raise RESTException(
                    'Invalid "limit" parameter - %s' % self.request.GET.get('limit'))
        ...
        if not self.request.GET.get('cursor'):
            ...
            try:
                ...
                raise RESTException(
                    'Invalid "cursor" argument - %s' % self.request.GET.get('cursor'))

        try:
            (results, cursor, more_available) = query.fetch_page(
                limit, start_cursor=cursor)
        except BadRequestError, exc:
            # This happens when we're using an existing cursor and the other
            # query arguments were messed with
            raise RESTException(
                'Invalid "cursor" argument - %s' % self.request.GET.get('cursor'))
    ...
    def _order_query(self, query):
        ...
        if not self.request.GET.get('order'):
            ...
            try:
                ...
                orders = [o.strip()
                          for o in self.request.GET.get('order').split(',')]
                orders = [
                    '+' + o if not o.startswith('-') and not o.startswith('+') else o for o in orders]

                # Translate property names (if it's defined for the current
                # model) - e.g. input 'col1' is actually 'my_col1' in MyModel
                translated_orders = dict(
                    [order.lstrip('-+'), order[0]] for order in orders)
                translated_orders = translate_property_names(
                    translated_orders, self.model, 'input')

                orders = [-getattr(self.model, order) if direction == '-' else getattr(
                    self.model, order) for order, direction in translated_orders.iteritems()]

            except AttributeError, exc:
                # Invalid column name
                raise RESTException(
                    'Invalid "order" parameter - %s' % self.request.GET.get('order'))
    ...
    def _build_model_from_data(self, data, cls, model=None):
        ...
        # Translate the property names (this is done before the filtering in
        # order to get the original property names by which the filtering is
        # done)
        ...
        # into another dict (so any other unauthorized properties will be
        # ignored).
        input_properties = {}
        for (name, prop) in cls._properties.iteritems():
            if name not in data:
                continue  # Input not given by user

            if prop._repeated:
                # This property is repeated (i.e. an array of values)
                input_properties[name] = [
                    self._value_to_property(value, prop) for value in data[name]]
            else:
                input_properties[name] = self._value_to_property(
                    data[name], prop)
        ...
        input_properties = dict(
            (k, v) for k, v in input_properties.iteritems() if k in included_properties)
        ...
        # and not the self.model class, since we need to support when a model
        # has an inner StructuredProperty, and that model has its own RESTMeta
        # definition.
...
def get_rest_class(ndb_model, base_url, **kwd):
    ...
    class RESTHandlerClass(BaseRESTHandler, blobstore_handlers.BlobstoreUploadHandler, blobstore_handlers.BlobstoreDownloadHandler):
        ...
        allow_http_method_override = kwd.get(
            'allow_http_method_override', True)
        allowed_origin = kwd.get('allowed_origin', None)

        # Wrapping in a list so the functions won't be turned into bound
        # methods
        ...
        if PERMISSION_OWNER_USER in permissions.values():
            if not hasattr(model, 'RESTMeta') or not hasattr(model.RESTMeta, 'user_owner_property'):
                raise ValueError(
                    'Must define a RESTMeta.user_owner_property for the model class %s if user-owner permission is used' % (model))
            if not hasattr(model, model.RESTMeta.user_owner_property):
                raise ValueError('The user_owner_property "%s" (defined in RESTMeta.user_owner_property) does not exist in the given model %s' % (
                    model.RESTMeta.user_owner_property, model))

        def __init__(self, request, response):
            self.initialize(request, response)
            blobstore_handlers.BlobstoreUploadHandler.__init__(
                self, request, response)
            blobstore_handlers.BlobstoreDownloadHandler.__init__(
                self, request, response)
            ...
            def inner_f(self, model_id, property_name=None):
                ...
                try:
                    ...
                    if model_id:
                        ...
                        model = self._model_id_to_model(model_id.lstrip('/'))

                        if (permission == PERMISSION_OWNER_USER) and (self.get_model_owner(model) != self.user.key):
                            # The currently logged-in user is not the owner of
                            # the model
                            return self.permission_denied()

                        if property_name and model:
                            # Get the original name of the property
                            property_name = translate_property_names(
                                {property_name: True}, model, 'input').keys()[0]
                    ...
                    if isinstance(result, webapp2.Response):
                        # webapp2.Response instance - no need for further
                        # manipulation (return as-is)
                        return result
                    elif not isinstance(result, NoResponseResult):
                        # Only return a result (i.e. write to the response
                        # object) if it's not a NoResponseResult (used when
                        # serving blobs - BlobKeyProperty)
        ...
        def get(self, model, property_name=None):
            ...
            if not model:
                ...
                if self.permissions['GET'] == PERMISSION_OWNER_USER:
                    ...
                    query = query.filter(
                        getattr(self.model, self.user_owner_property) == self.user.key)
                ...
                if property_name:
                    # Return a specific property value - currently supported
                    # only for BlobKeyProperty
                    if not hasattr(model, property_name):
                        raise RESTException(
                            'Invalid property name "%s"' % property_name)
                    ...
                    if not isinstance(blob_key, blobstore.BlobKey):
                        raise RESTException(
                            '"%s" is not a BlobKeyProperty' % property_name)

                    # Send the blob contents
                    self.send_blob(blob_key)

                    # Make sure we don't return a value (i.e. not write to
                    # self.response) - so self.send_blob will work properly
        ...
        def post(self, model, property_name=None):
            ...
            if model and property_name:
                ...
                if not upload_files:
                    ...
                    # every time the user GETs to /my_model - since each
                    # create_upload_url call creates more DB garbage.
                    upload_url = blobstore.create_upload_url(self.request.url)
                    # We use a 307 redirect in order to tell the client (e.g.
                    # browser) to use the same method type (POST) and keep its
                    # POST data
                    return self.redirect(upload_url, code=307)

                blob_info = upload_files[0]

                if getattr(model, property_name):
                    # The property already has a previous value - delete the
                    # older blob
            ...
            for model_to_create in json_data:
                try:
                    # Any exceptions raised due to invalid/missing input will
                    # be caught
        ...
        def put(self, model, property_name=None):
            ...
            if model:
                ...
                for model_to_update in json_data:
                    ...
                    model = self._build_model_from_data(
                        model_to_update, self.model, model)
                    models.append(model)
        ...
        def delete(self, model, property_name=None):
            ...
            if model:
                ...
                # Delete the models (we might need to fetch several pages in
                # case of many results)
                cursor = None
                more_available = True

                while more_available:
                    results, cursor, more_available = query.fetch_page(
                        BaseRESTHandler.DEFAULT_MAX_QUERY_RESULTS, start_cursor=cursor)
                    if results:
            ...
            for m in models:
                # No easy way to delete blobstore entries in a transaction
        ...
        def is_user_admin(self):
            ...
            if not hasattr(self.user, 'RESTMeta') or not hasattr(self.user.RESTMeta, 'admin_property'):
                ...
                # rest of the arguments are being validated) since at that
                # stage we can't see the webapp2 auth configuration to
                # determine the User model.
                raise ValueError('The user model class %s must include a RESTMeta class with `admin_property` defined' % (
                    self.user.__class__))

            admin_property = self.user.RESTMeta.admin_property
            if not hasattr(self.user, admin_property):
                raise ValueError('The user model class %s does not have the property %s as defined in its RESTMeta.admin_property' % (
                    self.user.__class__, admin_property))
...
# We inherit from NamePrefixRoute so the same router can actually return
# several routes simultaneously (used for BlobKeyProperty)
class RESTHandler(NamePrefixRoute):
    ...
    def __init__(self, url, model, **kwd):
        ...
        routes = [
            ...
            webapp2.Route(
                url + '<model_id:(/.+)?|/>', get_rest_class(model, url, **kwd), 'main')
        ]
        ...
        for (name, prop) in model._properties.iteritems():
            if isinstance(prop, ndb.BlobKeyProperty) and name in included_properties:
                ...
                # e.g. /api/my_model/<SOME_KEY>/blob_prop
                blob_property_url = '%s/<model_id:.+?>/<property_name:%s>' % (
                    url, property_name)

                # Upload/Download blob route and handler
                routes.insert(0, webapp2.Route(
                    blob_property_url, get_rest_class(model, url, **kwd), 'upload-download-blob'))
